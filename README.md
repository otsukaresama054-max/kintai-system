...
社員ごとに「前月21日〜当月20日」の出退勤記録をPDF1枚にまとめて出力する機能です。
実働時間の自動計算(出勤・退勤のペアリング)は行わず、日付・区分・時刻・簡易住所の
一覧のみを出力します。メール送信等は行わず、Googleドライブに保存して完結します。
...
4. 完了メッセージに表示されるリンクから、Googleドライブの
   **「勤怠PDF」>「(年)年(月)月分」** フォルダを開く
5. 社員の氏名ごとに1つずつPDFファイルが作成されている

### 注意点

- 「(拒否)」の区分(位置情報拒否の記録)はPDFには含まれません。
- 対象期間内に打刻が1件もない社員は、PDFが作成されません。
- 生成の都度、同名のPDFが増えていきます(上書きはしません)。
  同じ月を出力し直したい場合は、ドライブ上の古いファイルを手動で削除してください。
