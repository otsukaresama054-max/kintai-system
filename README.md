...
> `備考`列(J)には、退勤ボタンの下にある任意入力欄「最終訪問先」に
> 入力された内容が記録されます(未入力なら空欄のまま、出勤時は使用しません)。
> `コメント`列(K)には、出勤・退勤どちらでも使える自由記入の「備考」欄の
> 内容が記録されます。どちらも月次PDFに列として出力されます。
