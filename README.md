...
その場で開いて印刷する運用を想定しており、データを長期保管する前提では
ありません。メール送信等は行わず、印刷し終わったらドライブ上のファイルは
削除してもらって問題ありません(同じ月を再出力すると自動的に上書きされます)。
...
4. 完了メッセージに表示されるリンクを開くと、社員ごとにページが
   分かれた1つのPDF(`2026年09月分_出退勤記録.pdf`)が開けるので、
   そのまま印刷する
5. 印刷し終わったら、ファイルはドライブから削除してもらって構わない
   (Googleドライブの「勤怠PDF」フォルダに保存されている)
...
- 同じ月をもう一度出力すると、前回のファイルは自動的に削除されて
  1件だけが残ります(履歴は蓄積されません)。
