...
その場で開いて印刷する運用を想定しており、**Googleドライブへの保存は
一切行いません**。生成したPDFはポップアップ画面の中に直接埋め込んで
表示するだけなので、ダイアログを閉じれば何も残りません。
...
2. 上部メニューの **「勤怠帳票」** から **「月次PDFを出力」** を選ぶ
   - 初回はGoogle ドキュメント作成等のアクセス許可を求められるので許可する
3. 「対象の締め月」を `2026-09` のような形式で入力する
   (前月21日〜入力した月の20日、が対象期間になる)
4. ポップアップ画面の中に、社員ごとにページが分かれたPDFがそのまま
   表示されるので、そこから印刷する(ブラウザのPDFビューアに出る
   印刷アイコンや、右クリック→印刷などを利用する)
5. 印刷し終わったらポップアップを閉じるだけでよい(ドライブ等に
   ファイルは一切残らない)
...
- ドライブに保存しない設計のため、後から見返したい場合はその都度
  「月次PDFを出力」をやり直してください。
